// https://github.com/KalininPavel74/Java_HomeWorkUnit05.git
...
public class JavaUnit05KalininPavel {

    static Logger logger = Logger.getLogger(JavaUnit05KalininPavel.class.getName());
    ...
    static public void case1() {
        ...
        logger.warning(dwf.print(DiscreteWorkingField.FENCE, findWays.ways));
    }
...
*/        
        ...
        public String toString() {
            ...
            sb.append(" ").append(cellSeparator);
            for(int i=0; i<widthMap; i++) 
                sb.append(i).append(cellSeparator);
            ...
            for(int i=0; i<highMap; i++) {
                sb.append(i).append(" |").append(cellSeparator);
                for(int j=0; j<widthMap; j++) 
                    sb.append(getValue(j, i)).append(cellSeparator);
                sb.append(lineSeparator).append("  |").append(lineSeparator).append("  |").append(lineSeparator).append("  |").append(lineSeparator);
            }
            sb.append("high");
            return sb.toString();
        }

        // Печать карты без определеного элемента - для улучшения визуализации
        public String print(int escapeElement) {
            int len = ar.length;
            StringBuilder sb = new StringBuilder(len*3);
            String cellSeparator = "\t";
            String lineSeparator = System.getProperty("line.separator");
            ...
            for(int i=0; i<highMap; i++) {
                ...
                for(int j=0; j<widthMap; j++) { 
                    ...
                    String s = (temp == escapeElement)?" ":String.valueOf(temp);  
                    sb.append(s).append(cellSeparator);
                }    
                sb.append(lineSeparator).append("  |").append(lineSeparator).append("  |").append(lineSeparator).append("  |").append(lineSeparator);
            }
            sb.append("high");
        ...
        // Печать карты без определеного элемента - для улучшения визуализации
        // и с отображением вариантов ближайших путей от начала до выходов.
        public String print(int escapeElement, ArrayList<ArrayList<Cell>> ways) {
            int len = ar.length;
            StringBuilder sb = new StringBuilder(len*3);
            String cellSeparator = "\t";
            String lineSeparator = System.getProperty("line.separator");
